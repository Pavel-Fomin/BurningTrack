...
- не обходить все треклисты при построении каждой строки;
...
Старая реализация `DefaultTrackListBadgeProvider` считала бейджи напрямую из storage-слоя:

```text
LibraryTracksViewModel
→ DefaultTrackListBadgeProvider.badges(for:)
→ список meta треклистов
→ состав каждого треклиста
→ результат для UI
```

Такой подход смешивал отображение бейджей с полным чтением треклистов.

Проблемы:

- provider повторно загружал meta треклистов;
- provider проходил по составу всех треклистов;
- расчёт бейджей зависел от скорости storage-операций;
...
В новой архитектуре чтение треклистов остаётся в manager/storage-слое, а UI получает бейджи через индекс.
...
- списка meta треклистов;
- состава треклистов;
- поля `track.trackId` у элементов треклиста.

Обход треклистов выполняется только во время построения или перестроения индекса. Он не должен выполняться на каждый запрос бейджей из UI.
...
- принять `[UUID]` треков;
- вернуть `[UUID: [String]]`;
- не знать о физической схеме хранения треклистов;
- не выполнять storage-чтение при каждом вызове.

`DefaultTrackListBadgeProvider` должен использовать `TrackListBadgeIndex`, а не читать треклисты напрямую.
...
- очистки треклиста;
- сохранения строк конкретного треклиста в SQLite.
...
`TrackListBadgeIndex` отделяет бейджи треклистов от storage-чтения и от загрузки фонотеки.

Финальная архитектура:

- storage-слой сохраняет треклисты в SQLite и публикует события;
