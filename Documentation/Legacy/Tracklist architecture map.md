...
Роль: хранение и загрузка состава SQLite-очереди плеера.

Отвечает за:
    •    загрузку SQLite-очереди плеера;
...
Отвечает за:
    •    SQLite-строки состава одного треклиста;
...
Пока проект живёт на нескольких runtime-реестрах, всегда есть риск расхождения между:
    •    TrackRegistry
    •    BookmarksRegistry
    •    TrackIdentityRegistry
    •    SQLite-очередью плеера
    •    SQLite meta треклистов
    •    SQLite-строками состава треклистов
