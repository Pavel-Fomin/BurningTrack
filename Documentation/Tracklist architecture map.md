...
Отвечает за:
    ...
    •    работу мини-плеера;
    •    загрузку metadata/artwork для текущего трека.

Ключевой принцип:
PlayerViewModel управляет сценарием воспроизведения, а PlayerManager — самим проигрывателем.
...
Не отвечает за:
    •    воспроизведение;
    •    AVPlayer;
    •    metadata cache как источник истины.
...
Ключевой принцип:
Треклист не решает файловый доступ сам, а опирается на BookmarkResolver.
...
Роль: runtime-кэш метаданных и artwork.

Отвечает за:
    •    чтение title / artist / duration / artwork;
    •    кэширование metadata;
    •    ускорение UI и плеера.

Не отвечает за:
    •    постоянное хранение треков;
    •    структуру библиотеки;
    •    trackId.

Ключевой принцип:
Это runtime-слой, а не источник истины.
...
Metadata и artwork — runtime-данные, а не источник истины.
